...
class TestFeature(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        feature_service = api.services.create_feature_service(f'Testing Feature {random.randint(0, 99)}', 'This is a test.')
        cls.api = api
    ...
    def test_create_point_feature(self):
        ...
        fl = TestFeature.create_point_feature_layer('Test Create Feature', api, feature_service)
    ...
    def test_get_features(self):
        ...
        point_feature1 = api.services.add_point(lon=10.0, lat=20.0, layer_url=fl.url, attributes=attributes)
        point_feature2 = api.services.add_point(lon=10.2, lat=20.2, layer_url=fl.url, attributes=attributes)
        point_feature3 = api.services.add_point(lon=10.4, lat=20.4, layer_url=fl.url, attributes=attributes)

        features = api.services.get_features(feature_service.url, fl.id, "DeviceId = 'abc123'", ['OBJECTID'])
    ...
    def test_get_features_bad_where(self):
        ...
        fl = TestFeature.create_point_feature_layer('Test Create Feature - Bad Where', api, feature_service)
        ...
        with self.assertRaises(saw.exceptions.ArcGISException):
            features = api.services.get_features(feature_service.url, fl.id, "asdf = 100", ['OBJECTID'])
    ...
    def test_get_features_bad_out_fields(self):
        ...
        fl = TestFeature.create_point_feature_layer('Test Create Feature - Bad Out Fields', api, feature_service)
        ...
        with self.assertRaises(saw.exceptions.ArcGISException):
            features = api.services.get_features(feature_service.url, fl.id, "DeviceId = 'abc123'", ['asdf', 'qwerty'])
    '''
    ...
    def test_update_features(self):
        ...
        fl = TestFeature.create_point_feature_layer('Test Update Features', api, feature_service)
        ...
        updates = [(f.id, {'Name': 'Casey Jones'}, None) for f in features]
